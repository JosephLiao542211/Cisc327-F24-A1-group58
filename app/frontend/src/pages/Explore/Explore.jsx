...
const Explore = () => {
    ...
    const applyFilters = (flight) => {
        ...
        const matchesDate = dateFilter ? new Date(flight.departureTime).toLocaleDateString('en-US') === new Date(dateFilter).toLocaleDateString('en-US') : true;
        const matchesPrice = (minPrice ? flight.economyPrice >= minPrice : true) && (maxPrice ? flight.economyPrice <= maxPrice : true);
        const matchesDepartureAirport = departureAirportFilter ? flight.departureAirport.toLowerCase().includes(departureAirportFilter.toLowerCase()) : true;
        const matchesArrivalAirport = arrivalAirportFilter ? flight.arrivalAirport.toLowerCase().includes(arrivalAirportFilter.toLowerCase()) : true;
        return matchesDiscount && matchesDate && matchesPrice && matchesDepartureAirport && matchesArrivalAirport;
    };
                    ...
                    {filteredFlights.length > 0 ? (
                        filteredFlights.map((flight) => (
                            <FlightCard
                                key={flight.id}
                                id={flight.id}
                                imageURL={airportImageCatalogue[flight.arrivalAirport]?.image}
                                description={`${new Date(flight.departureTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} \u00A0\u00A0\u00A0 ${flight.departureAirport} → ${flight.arrivalAirport}`}
                                location={airportImageCatalogue[flight.arrivalAirport]?.location}
